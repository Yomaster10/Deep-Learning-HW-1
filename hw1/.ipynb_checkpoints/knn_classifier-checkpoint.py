...
class KNNClassifier(object):
    ...
    def predict(self, x_test: Tensor):
        ...
        for i in range(n_test):
            ...
            # ====== YOUR CODE: ======
            # Step 1
            kNN = torch.topk(input=dist_matrix[:,i], k=self.k, largest=False)
            idx = kNN.indices
            
            # Step 2
            labels = self.y_train[idx]
            y_pred[i] = torch.mode(labels)[0]
            # ========================
...
def l2_dist(x1: Tensor, x2: Tensor):
    ...
    dists = None
    # ====== YOUR CODE: ======
    sq_x1 = torch.square(x1)
    sq_x2 = torch.square(x2)
    
    sq_x1_sum = torch.sum(sq_x1, 1, keepdim=True)
    sq_x2_sum = torch.sum(sq_x2, 1)
    
    sq_x1_exp = sq_x1_sum.expand(-1, x2.shape[0])
    sq_x2_exp = sq_x2_sum.expand(x1.shape[0], -1)
    
    x1x2 = x1 @ torch.transpose(x2,0,1)
    
    dists = torch.sqrt(sq_x1_exp - (2*x1x2) + sq_x2_exp)
    # ========================
...
def find_best_k(ds_train: Dataset, k_choices, num_folds):
    ...
    for i, k in enumerate(k_choices):
        ...
        # ====== YOUR CODE: ======
        val_ratio = 1/num_folds
        acc_list = [0]*num_folds

        for j in range(num_folds):
            # Split & Train
            dl_train, dl_val = dataloaders.create_train_validation_loaders(ds_train, val_ratio)
            model.train(dl_train)
              
            # Validate
            x_val, y_val = next(iter(dl_val))
            y_pred = model.predict(x_val)
            acc_list[j] = accuracy(y_val, y_pred)

        # Return Accuracy Scores for k
